...
import com.tocersoft.base.constant.DeviceConstant;
...
public class HttpRequestDeviceUtil {
     ...
     * PAD浏览器
     */
    private static String[] padUserAgents = new String[]{
...
//		if(1==1){
//			return DeviceConstant.PAD;
//		}

        //手机
        for (int i = 0; via != null && !via.trim().equals("") && i < mobileGateWayHeaders.length; i++) {
            if (via.contains(mobileGateWayHeaders[i])) {
                return DeviceConstant.MOBILE;
            }
        }
        ...
        //PAD
        for (int i = 0; userAgent != null && userAgent != null && !userAgent.trim().equals("") && i < padUserAgents.length; i++) {
            if (userAgent.contains(padUserAgents[i])) {
                return DeviceConstant.PAD;
            }
        }
